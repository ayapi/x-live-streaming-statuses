...
- [x] 10. わんコメコメント送信のバグ修正
- [x] 10.1 タイムスタンプの型修正と400エラーハンドリングの改善
  - わんコメに送信するコメントのタイムスタンプを数値型（エポックミリ秒）のまま送信するよう修正する（現在の文字列変換を削除し、わんコメのAJVスキーマが要求する`oneOf[number, date-time string]`のnumber側に適合させる）
  - わんコメ送信用のコメント型定義において、タイムスタンプのフィールド型を文字列から数値に変更する
  - 送信エラーの型にバリデーションエラーのバリアントを追加し、枠ID無効エラーと区別可能にする
  - わんコメから400レスポンスを受信した際、レスポンスボディをJSON解析してエラーの原因を特定する: 枠IDが存在しないのか、コメントデータのスキーマバリデーションに失敗したのかを判別する
  - バリデーションエラーの場合はエラー詳細（どのフィールドが不正か）をログ出力し、枠ID無効の場合は従来どおり設定確認を促すメッセージを表示する
  - バッファリングクライアントにおいて、バリデーションエラーもリトライ不要エラーとして扱い、無駄なリトライを防止する
  - _Requirements: 2.1, 2.2, 2.4, 4.4_

- [x] 10.2 修正に伴うテストの更新
  - わんコメAPIへの送信テストを修正し、タイムスタンプが数値型のまま送信されることを検証する（文字列型の検証を数値型に変更）
  - 400レスポンスがコメントデータのバリデーションエラーである場合に、正しいエラー種別が返されることを検証する新規テストを追加する
  - 400レスポンスが枠ID無効である場合の既存テストが引き続き正しく動作することを確認する
  - バッファリングクライアントのテストに、バリデーションエラー時のリトライスキップ動作を検証するケースを追加する
  - パイプライン統合テストで、タイムスタンプが数値型としてわんコメに到達することを確認する
